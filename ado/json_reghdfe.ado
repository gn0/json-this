*! version 1.0 17aug2018

capture program drop json_reghdfe
program define json_reghdfe
    ...
    reghdfe `anything' if `to_use' `weight_exp', `options'
    ...
    if (~missing("`using'")) {
        ...
        local output `"`output' "absvars": "`e(absvars)'","'
        ...
        local output `"`output' "r2_within": `e(r2_within)',"'
        local output `"`output' "r2_a": `e(r2_a)',"'
        local output `"`output' "r2_a_within": `e(r2_a_within)',"'

        if (strpos("`anything'", "(")) {
            /* Cragg-Donald Wald F statistic */
            local output `"`output' "cdf": `e(cdf)',"'

            /* Hansen's J-stat p-value for overidentification. */
            local output `"`output' "jp": `e(jp)',"'
        }
